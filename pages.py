...
:root{--bg:#0b1a2e;--card:#0f1f33;--accent:#3b82f6;--text:#f0f9ff;--dim:#6b8db5;--mid:#9bb8da;--border:rgba(59,130,246,0.25);--input-bg:rgba(0,0,0,0.3)}
...
.bg{position:fixed;inset:0;background:radial-gradient(ellipse 80% 60% at 50% 0%,rgba(59,130,246,0.12),transparent 70%),var(--bg);z-index:0}
.grid{position:fixed;inset:0;background-image:linear-gradient(rgba(59,130,246,0.03) 1px,transparent 1px),linear-gradient(90deg,rgba(59,130,246,0.03) 1px,transparent 1px);background-size:44px 44px;z-index:0}
.orb{position:fixed;border-radius:50%;filter:blur(100px);z-index:0;animation:fl 10s ease-in-out infinite}
.o1{width:400px;height:400px;background:rgba(59,130,246,0.08);top:-120px;right:-100px}
.o2{width:300px;height:300px;background:rgba(16,185,129,0.04);bottom:-80px;left:-80px;animation-delay:5s}
...
.card{background:var(--card);border:1px solid var(--border);border-radius:24px;padding:40px 36px 36px;backdrop-filter:blur(20px);box-shadow:0 0 80px rgba(59,130,246,0.08),0 20px 60px rgba(0,0,0,0.6)}
.brand{display:flex;align-items:center;gap:14px;margin-bottom:28px}
.brand-img{width:48px;height:48px;border-radius:50%;overflow:hidden;border:1px solid var(--border);box-shadow:0 0 24px rgba(59,130,246,0.3);flex-shrink:0}
...
.sub{font-size:13px;color:var(--mid);margin-bottom:26px;line-height:1.5}
.hint{display:flex;align-items:center;gap:12px;background:rgba(59,130,246,0.06);border:1px solid rgba(59,130,246,0.12);border-radius:12px;padding:10px 16px;margin-bottom:22px}
.hint-label{font-size:11px;color:var(--dim);flex:1}
.hint-val{font-family:ui-monospace,monospace;font-size:13px;font-weight:700;color:var(--accent);background:rgba(59,130,246,0.08);border:1px solid rgba(59,130,246,0.2);padding:4px 12px;border-radius:8px;cursor:pointer;transition:.15s;letter-spacing:.06em}
.hint-val:hover{background:rgba(59,130,246,0.18)}
.field{margin-bottom:18px}
.field label{display:block;font-size:11px;font-weight:600;color:var(--mid);margin-bottom:6px;text-transform:uppercase;letter-spacing:.05em}
.inp-wrap{position:relative}
input{width:100%;padding:13px 44px 13px 16px;border-radius:12px;border:1px solid var(--border);background:var(--input-bg);color:var(--text);font-family:inherit;font-size:14px;outline:none;transition:.2s}
input:focus{border-color:rgba(59,130,246,0.6);background:rgba(0,0,0,0.4);box-shadow:0 0 0 3px rgba(59,130,246,0.08)}
...
.toggle{position:absolute;right:14px;top:50%;transform:translateY(-50%);background:none;border:none;color:var(--dim);cursor:pointer;font-size:18px;padding:0;transition:.2s}
.toggle:hover{color:var(--text)}
.err{display:none;background:rgba(239,68,68,.08);border:1px solid rgba(239,68,68,.2);border-radius:12px;padding:10px 14px;margin-bottom:16px;font-size:12px;color:#F87171;align-items:center;gap:8px}
.err.show{display:flex}
.btn{width:100%;padding:13px;border-radius:12px;border:none;cursor:pointer;background:linear-gradient(135deg,#3b82f6,#8b5cf6);color:#fff;font-family:inherit;font-size:15px;font-weight:600;display:flex;align-items:center;justify-content:center;gap:8px;box-shadow:0 4px 24px rgba(139,92,246,0.3);transition:.2s;position:relative;overflow:hidden}
.btn::before{content:'';position:absolute;inset:0;background:rgba(255,255,255,.06);opacity:0;transition:.2s}
.btn:hover::before{opacity:1}
.btn:disabled{opacity:.5;cursor:not-allowed}
.links{display:flex;justify-content:space-between;margin-top:16px;font-size:12px;color:var(--dim)}
.links a{color:var(--accent);text-decoration:none;font-weight:500;transition:.15s}
.links a:hover{opacity:.8}
.footer{margin-top:24px;padding-top:18px;border-top:1px solid var(--border);display:flex;align-items:center;justify-content:center;gap:8px;font-size:11px;color:var(--dim)}
...
<div class="wrap">
  <div class="card">
    ...
    <h1>خوش آمدید</h1>
    <p class="sub">برای ورود به پنل، ایمیل و رمز عبور خود را وارد کنید</p>
    <div class="err" id="err"><span>⚠️</span><span id="err-text"></span></div>
    <div class="hint">
      <span class="hint-label">رمز پیش‌فرض سیستم</span>
      <span class="hint-val" onclick="document.getElementById('email').value='admin@x4g.local';document.getElementById('pw').value='X4GKING'">X4GKING</span>
    </div>
    <form id="form">
      <div class="field">
        <label>ایمیل</label>
        <div class="inp-wrap">
          <input type="email" id="email" placeholder="example@domain.com" value="admin@x4g.local" required>
      ...
      <div class="field">
        ...
        <div class="inp-wrap">
          ...
          <span class="ic">🔒</span>
          <button type="button" class="toggle" id="togglePw">👁️</button>
        </div>
      </div>
      <button class="btn" type="submit" id="btn">🔑 ورود</button>
    </form>
    <div class="links">
      <a href="#">🔑 رمز را فراموش کرده‌اید؟</a>
      <a href="#">➕ ثبت‌نام</a>
    </div>
    <div class="footer">پشتیبانی <a href="https://t.me/X4GHUB" target="_blank">📱 @X4GHUB</a></div>
  </div>
</div>
<script>
...
document.getElementById('form').addEventListener('submit', function(e) {
  ...
  err.classList.remove('show'); btn.disabled = true; btn.innerHTML = '⏳ در حال ورود...';
  const email = document.getElementById('email').value.trim();
  const pass = document.getElementById('pw').value.trim();
  setTimeout(() => {
    if (email === 'admin@x4g.local' && pass === 'X4GKING') {
      window.location.href = '/dashboard';
    } else {
      et.textContent = 'ایمیل یا رمز عبور اشتباه است';
