...
def get_public_page_html(uuid_key: str) -> str:
    """صفحه پابلیک ساب — طراحی یکپارچه با پشتیبانی"""
...
.wrap{position:relative;z-index:10;max-width:820px;margin:0 auto;padding:24px 16px 64px;animation:fadeSlide 0.8s ease}
...
/* ── بخش بالایی (گروه دسترسی) ── */
.sub-info{background:var(--glass);backdrop-filter:blur(24px);border:1px solid var(--glass-border);border-radius:var(--radius);padding:22px 24px 20px;margin-bottom:16px;box-shadow:var(--shadow);position:relative;overflow:hidden;transition:var(--transition)}
...
.sub-name{font-size:22px;font-weight:800;color:var(--t1);margin-bottom:4px;letter-spacing:-0.02em}
.sub-desc{font-size:12px;color:var(--t2);line-height:1.8;margin-bottom:12px}
.sub-meta-row{font-size:10px;color:var(--t3);margin-bottom:12px;display:flex;align-items:center;gap:6px;flex-wrap:wrap}
.sub-sub-box{background:var(--accent-d);border:1px solid var(--glass-border);border-radius:12px;padding:10px 14px;display:flex;align-items:center;gap:8px;flex-wrap:wrap}
.sub-sub-url{font-family:ui-monospace,monospace;font-size:9.5px;color:var(--accent2);word-break:break-all;flex:1;min-width:120px}

/* ── آمار ── */
.stats-bar{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:16px}
.stat-card{background:var(--glass);backdrop-filter:blur(16px);border:1px solid var(--glass-border);border-radius:14px;padding:14px 16px;transition:var(--transition)}
.stat-card:hover{border-color:rgba(91,141,239,0.08);transform:translateY(-2px)}
.stat-label{font-size:8.5px;color:var(--t3);font-weight:700;text-transform:uppercase;letter-spacing:0.07em;margin-bottom:6px}
.stat-val{font-size:20px;font-weight:800;color:var(--t1);line-height:1;letter-spacing:-0.01em}
.stat-sub{font-size:9px;color:var(--t3);margin-top:5px}
.dot{width:5px;height:5px;border-radius:50%;background:var(--green);display:inline-block;animation:pulse 2s infinite}
@keyframes pulse{0%,100%{opacity:1}50%{opacity:0.25}}

/* ── دکمه‌ها ── */
.btn{font-family:inherit;font-size:11px;font-weight:700;border-radius:10px;padding:7px 14px;cursor:pointer;display:inline-flex;align-items:center;gap:5px;border:none;transition:all 0.25s cubic-bezier(0.34,1.56,0.64,1);white-space:nowrap}
.btn i{font-size:12px}
.btn-p{background:linear-gradient(135deg,var(--accent),#7a5cf0);color:#fff;box-shadow:0 4px 16px rgba(91,141,239,0.12)}
.btn-p:hover{transform:translateY(-2px);box-shadow:0 8px 24px rgba(91,141,239,0.18)}
...
.btn-sm{padding:4px 8px;font-size:9.5px;border-radius:6px}
.btn-sm i{font-size:10px}

/* ── جدول عمودی ── */
.cfg-title{font-size:12px;font-weight:800;color:var(--t2);margin-bottom:12px;display:flex;align-items:center;gap:6px;text-transform:uppercase;letter-spacing:0.07em}
...
.cfg-table{width:100%;border-collapse:collapse;font-size:12px;min-width:600px}
.cfg-table td{padding:10px 14px;border-bottom:1px solid var(--glass-border);vertical-align:middle}
.cfg-table tr:last-child td{border-bottom:none}
.cfg-table tr:hover td{background:var(--accent-d)}
.cfg-table .label-cell{color:var(--t2);font-weight:600;width:110px;white-space:nowrap;font-size:11px}
.cfg-table .label-cell i{color:var(--accent);font-size:13px;margin-left:6px}
.cfg-table .value-cell{color:var(--t1);font-weight:500}
.cfg-table .value-cell .proto-badge{font-size:8.5px;padding:2px 8px;border-radius:6px;font-weight:700;background:var(--accent-d);color:var(--accent2)}
.cfg-table .value-cell .proto-badge.xhttp{background:var(--purple-bg);color:var(--purple-t)}
.cfg-table .value-cell .status-dot{width:7px;height:7px;border-radius:50%;display:inline-block;margin-left:5px}
.cfg-table .value-cell .status-dot.active{background:var(--green);box-shadow:0 0 8px rgba(31,184,126,0.3)}
.cfg-table .value-cell .status-dot.inactive{background:var(--red);box-shadow:0 0 8px rgba(239,68,68,0.2)}
.cfg-table .value-cell .status-dot.expired{background:var(--amber);box-shadow:0 0 8px rgba(242,163,61,0.2)}
.cfg-table .value-cell .exp-chip{font-size:8.5px;padding:2px 8px;border-radius:6px;font-weight:700;white-space:nowrap;display:inline-block}
.cfg-table .value-cell .exp-ok{background:var(--green-bg);color:var(--green-t)}
.cfg-table .value-cell .exp-warn{background:var(--amber-bg);color:var(--amber-t)}
.cfg-table .value-cell .exp-exp{background:var(--red-bg);color:var(--red-t)}
.cfg-table .value-cell .exp-inf{background:var(--accent-d);color:var(--accent2)}
.cfg-table .value-cell .usage-bar{width:80px;height:4px;border-radius:3px;background:rgba(91,141,239,0.06);overflow:hidden;display:inline-block;vertical-align:middle;margin:0 5px}
.cfg-table .value-cell .usage-fill{height:100%;border-radius:3px;transition:width 0.6s ease}
.cfg-table .value-cell .actions-wrap{display:flex;gap:4px;flex-wrap:wrap}
.cfg-table .value-cell .vless-toggle{display:grid;grid-template-rows:0fr;transition:grid-template-rows 0.25s ease;overflow:hidden;margin-top:4px}
.cfg-table .value-cell .vless-toggle.open{grid-template-rows:1fr}
.cfg-table .value-cell .vless-inner{overflow:hidden}
.cfg-table .value-cell .vless-code{background:rgba(0,0,0,0.08);border-radius:6px;padding:5px 10px;font-size:9px;font-family:ui-monospace,monospace;color:var(--accent2);word-break:break-all;line-height:1.6;margin-top:4px}
...
.lock-banner{background:linear-gradient(150deg,rgba(91,141,239,0.04),rgba(91,141,239,0.01) 70%);padding:38px 30px 26px}
.lock-shield{width:64px;height:64px;border-radius:18px;background:var(--glass);border:1px solid var(--glass-border);display:flex;align-items:center;justify-content:center;margin:0 auto 16px;box-shadow:var(--shadow);position:relative}
...
.lock-inp{width:100%;padding:13px 44px 13px 44px;border-radius:13px;border:1.5px solid var(--glass-border);background:rgba(255,255,255,0.01);color:var(--t1);font-family:inherit;font-size:14px;outline:none;text-align:center;letter-spacing:0.14em;transition:var(--transition)}
...
.qr-box{background:var(--glass);backdrop-filter:blur(28px);border:1px solid var(--glass-border);border-radius:22px;padding:26px;text-align:center;max-width:340px;width:100%;box-shadow:var(--shadow);animation:fadeSlide 0.4s ease}
.qr-title{font-size:13.5px;font-weight:800;margin-bottom:14px;color:var(--t1)}
...
/* ── فوتر با پشتیبانی ── */
.footer{text-align:center;padding-top:24px;display:flex;flex-direction:column;align-items:center;gap:8px}
.footer .support-btn{background:var(--glass);backdrop-filter:blur(12px);border:1px solid var(--glass-border);border-radius:12px;padding:10px 24px;color:var(--t2);font-family:inherit;font-size:11px;font-weight:600;cursor:pointer;display:inline-flex;align-items:center;gap:6px;transition:var(--transition)}
.footer .support-btn:hover{background:var(--accent-d);color:var(--accent2);border-color:rgba(91,141,239,0.12);transform:translateY(-1px)}
.footer .support-btn i{color:var(--accent);font-size:14px}
.footer .footer-links{display:flex;align-items:center;gap:12px;font-size:10px;color:var(--t3);flex-wrap:wrap;justify-content:center}
.footer .footer-links a{color:var(--accent2);font-weight:600;text-decoration:none;transition:var(--transition)}
.footer .footer-links a:hover{color:var(--accent)}
.copy-all-btn-wrap{margin-top:12px;display:flex;justify-content:center}
.copy-all-btn-wrap .btn{background:linear-gradient(135deg,var(--accent),#7a5cf0);color:#fff;padding:10px 24px;font-size:12.5px;border-radius:12px;box-shadow:0 4px 16px rgba(91,141,239,0.12)}
.copy-all-btn-wrap .btn:hover{transform:translateY(-2px);box-shadow:0 8px 24px rgba(91,141,239,0.18)}
...
@media(max-width:640px){
  ...
  .lock-banner{padding:30px 20px 22px}
  .lock-form{padding:18px 20px 24px}
  .sub-info{padding:16px 14px 14px}
  .cfg-table{font-size:11px;min-width:480px}
  .cfg-table td{padding:8px 10px}
  .cfg-table .label-cell{width:80px;font-size:10px}
}
@keyframes spin{to{transform:rotate(360deg)}}
</style>
</head>
...
<div class="wrap">
  ...
  <div class="footer">
    <button class="support-btn" onclick="window.open('https://t.me/X4GHUB','_blank')"><i class="ti ti-headset"></i> پشتیبانی</button>
...
function expChip(exp, expired){
  if(expired) return `<span class="exp-chip exp-exp">منقضی</span>`;
  if(!exp) return `<span class="exp-chip exp-inf">نامحدود</span>`;
  const d = Math.ceil((new Date(exp) - Date.now()) / 864e5);
  if(d <= 0) return `<span class="exp-chip exp-exp">منقضی</span>`;
  if(d <= 3) return `<span class="exp-chip exp-warn">${toFa(d)} روز</span>`;
  return `<span class="exp-chip exp-ok">${toFa(d)} روز</span>`;
}
function statusBadge(active, expired){
  if(expired) return `<span class="status-dot expired"></span> منقضی`;
  if(active) return `<span class="status-dot active"></span> فعال`;
  return `<span class="status-dot inactive"></span> غیرفعال`;
}
  ...
  document.getElementById('root').innerHTML = `
    <div class="sub-info">
      ...
      <div class="sub-sub-box">
        ...
        <button class="btn btn-pur btn-sm" onclick="navigator.clipboard.writeText(window._x4gSubUrl).then(()=>toast('لینک ساب کپی شد ✓','ok'))">
          <i class="ti ti-copy"></i> کپی لینک ساب
        </button>
        <button class="btn btn-g btn-sm" onclick="showQR(window._x4gSubName + ' — کل گروه', window._x4gSubUrl)">
    ...
    <div class="stats-bar">
      ...
      <div class="stat-card">
        ...
        <div class="stat-sub"><span class="dot"></span> آنلاین</div>
      </div>
      <div class="stat-card">
    ...
    <div class="cfg-title"><i class="ti ti-table"></i> لیست کانفیگ‌ها (${toFa(d.links.length)})</div>
      ...
      <table class="cfg-table">
        <tbody>
          ${d.links.map((l, i) => {
            ...
            return `
            <tr>
              <td class="label-cell"><i class="ti ti-checkbox"></i> وضعیت</td>
              <td class="value-cell">${statusBadge(l.active, l.expired)}</td>
            </tr>
            <tr>
              <td class="label-cell"><i class="ti ti-tag"></i> نام</td>
              <td class="value-cell">${esc(l.label)}${l.connections > 0 ? ` <span style="font-size:9px;color:var(--green-t);margin-right:6px">● ${toFa(l.connections)} اتصال</span>` : ''}</td>
            </tr>
            <tr>
              <td class="label-cell"><i class="ti ti-plug-connected"></i> پروتکل</td>
              <td class="value-cell">${protoBadge(l.protocol)}</td>
            </tr>
            <tr>
              <td class="label-cell"><i class="ti ti-gauge"></i> مصرف</td>
              <td class="value-cell">
                ${fmtB(l.used_bytes)}
            ...
            <tr>
              <td class="label-cell"><i class="ti ti-database"></i> سهمیه</td>
              <td class="value-cell">${lim}</td>
            </tr>
            <tr>
              <td class="label-cell"><i class="ti ti-calendar"></i> انقضا</td>
              <td class="value-cell">${expChip(l.expires_at, l.expired)}</td>
            </tr>
            <tr>
              <td class="label-cell"><i class="ti ti-tools"></i> عملیات</td>
              <td class="value-cell">
                <div class="actions-wrap">
                  <button class="btn btn-g btn-sm" onclick="navigator.clipboard.writeText('${esc(l.vless_link)}').then(()=>toast('لینک کپی شد ✓','ok'))" title="کپی لینک"><i class="ti ti-copy"></i> کپی</button>
                  <button class="btn btn-g btn-sm" onclick="showQR('${esc(l.label)}', '${esc(l.vless_link)}')" title="QR"><i class="ti ti-qrcode"></i> QR</button>
                  <button class="btn btn-g btn-sm" onclick="toggleVless(${i})" id="vt-${i}" title="نمایش لینک"><i class="ti ti-eye"></i></button>
                </div>
                <div class="vless-toggle" id="vw-${i}">
                  <div class="vless-inner">
    ...
    <div class="copy-all-btn-wrap">
      <button class="btn" onclick="copyAllConfigs()"><i class="ti ti-clipboard-copy"></i> کپی همه‌ی کانفیگ‌ها (${toFa(activeCount)})</button>
    </div>
  `;
  ...
  document.querySelectorAll('[id^="vt-"]').forEach(btn => {
    ...
    if(btn && wrap) {
      btn.onclick = function(e) {
        ...
        btn.innerHTML = open ? '<i class="ti ti-eye-off"></i>' : '<i class="ti ti-eye"></i>';
      };
    }
  });
