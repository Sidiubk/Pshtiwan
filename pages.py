...
<div class="wrap">
  <div class="card">
    ...
    <div class="footer" style="margin-top:28px;padding-top:20px;border-top:1px solid var(--glass-border);display:flex;align-items:center;justify-content:center;gap:10px;font-size:11px;color:var(--text-dim);transition:var(--transition)">
...
def get_public_page_html(uuid_key: str) -> str:
    """صفحه پابلیک ساب — مدرن‌ترین طراحی، سطح پنل‌های پولی"""
    
    html = r"""<!DOCTYPE html>
<html lang="fa" dir="rtl">
<head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0">
<title>X4G Sub · حرفه‌ای</title>
<link rel="preconnect" href="https://fonts.googleapis.com">
<link href="https://fonts.googleapis.com/css2?family=Vazirmatn:wght@300;400;500;600;700;800;900&display=swap" rel="stylesheet">
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/@tabler/icons-webfont@3.19.0/dist/tabler-icons.min.css">
<style>
/* ========================================
   طراحی درجه‌یک — سطح پنل‌های پولی
   ======================================== */
...
:root{
  ...
  --glass-shadow:0 20px 64px rgba(0,0,0,0.6);
  --card:rgba(255,255,255,0.03);
  --card-b:rgba(255,255,255,0.06);
  --card-bh:rgba(91,141,239,0.2);
  --accent:#5b8def;
  --accent2:#7aa3ff;
  --accent-d:rgba(91,141,239,0.08);
  --green:#1FB87E;
...
[data-theme="light"]{
  ...
  --glass-shadow:0 20px 64px rgba(0,0,0,0.05);
  --card:rgba(255,255,255,0.6);
  --card-b:rgba(0,0,0,0.06);
  --card-bh:rgba(46,99,214,0.2);
  --accent:#3b6fd4;
  --accent2:#5a88e8;
  --accent-d:rgba(46,99,214,0.06);
  --green:#0E9A6A;
  ...
  --t2:#48577a;
  --t3:#8694b0;
  --shadow:0 20px 64px rgba(0,0,0,0.05);
}

html,body{min-height:100%;background:var(--bg);font-family:'Vazirmatn',sans-serif;color:var(--t1);font-size:15px;transition:var(--transition);overflow-x:hidden}
.bg-fx{position:fixed;inset:0;background:radial-gradient(ellipse 70% 50% at 50% -10%,rgba(91,141,239,0.08),transparent 60%),var(--bg);z-index:0;transition:var(--transition)}
.particles{position:fixed;inset:0;z-index:0;pointer-events:none;overflow:hidden}
.particle{position:absolute;width:2px;height:2px;background:rgba(255,255,255,0.1);border-radius:50%;animation:twinkle var(--d) ease-in-out infinite alternate}
.particle:nth-child(1){top:10%;left:20%;--d:5s;animation-delay:0s}
.particle:nth-child(2){top:30%;right:15%;--d:6s;animation-delay:1.2s}
.particle:nth-child(3){bottom:25%;left:10%;--d:4.5s;animation-delay:0.8s}
.particle:nth-child(4){top:60%;right:25%;--d:5.5s;animation-delay:2s}
.particle:nth-child(5){bottom:35%;right:5%;--d:4s;animation-delay:1.5s}
.particle:nth-child(6){top:15%;left:50%;--d:6.5s;animation-delay:0.3s}
@keyframes twinkle{0%{opacity:0.02;transform:scale(0.5)}100%{opacity:0.3;transform:scale(1.2)}}
.wrap{position:relative;z-index:10;max-width:900px;margin:0 auto;padding:24px 16px 60px;animation:fadeSlide 0.9s cubic-bezier(0.16,1,0.3,1)}
@keyframes fadeSlide{from{opacity:0;transform:translateY(40px) scale(0.98)}to{opacity:1;transform:translateY(0) scale(1)}}

/* ── هدر حرفه‌ای ── */
.top{display:flex;align-items:center;justify-content:space-between;margin-bottom:28px;gap:12px;flex-wrap:wrap}
.brand{display:flex;align-items:center;gap:12px}
.brand-icon{width:42px;height:42px;border-radius:50%;background:linear-gradient(135deg,var(--accent),#7a5cf0);display:flex;align-items:center;justify-content:center;font-size:18px;font-weight:900;color:#fff;box-shadow:0 0 40px rgba(91,141,239,0.15)}
.brand-text{font-size:15px;font-weight:700;color:var(--t1);letter-spacing:-0.01em}
.brand-sub{font-size:10px;color:var(--t3);font-weight:500}
.top-actions{display:flex;align-items:center;gap:6px;flex-shrink:0;flex-wrap:wrap}
.icon-btn{width:36px;height:36px;border-radius:12px;background:var(--glass);backdrop-filter:blur(12px);border:1px solid var(--glass-border);color:var(--t2);display:flex;align-items:center;justify-content:center;font-size:16px;cursor:pointer;transition:var(--transition)}
.icon-btn:hover{background:var(--accent-d);color:var(--accent2);border-color:rgba(91,141,239,0.15);transform:translateY(-2px)}
.lang-btn{font-size:11px;font-weight:700;padding:0 14px;height:36px;border-radius:12px;background:var(--glass);backdrop-filter:blur(12px);border:1px solid var(--glass-border);color:var(--t2);cursor:pointer;display:flex;align-items:center;gap:4px;transition:var(--transition);font-family:inherit}
.lang-btn:hover{background:var(--accent-d);color:var(--accent2);border-color:rgba(91,141,239,0.15)}
.theme-picker{display:flex;align-items:center;gap:4px;background:var(--glass);backdrop-filter:blur(12px);border:1px solid var(--glass-border);border-radius:12px;padding:4px 6px}
.theme-option{width:20px;height:20px;border-radius:50%;border:2px solid var(--glass-border);cursor:pointer;transition:all 0.3s cubic-bezier(0.34,1.56,0.64,1);position:relative}
.theme-option:hover{transform:scale(1.15);border-color:var(--t2)}
.theme-option.active{border-color:var(--accent);box-shadow:0 0 16px rgba(91,141,239,0.2);transform:scale(1.1)}
.theme-option .tooltip{display:none;position:absolute;bottom:calc(100% + 6px);left:50%;transform:translateX(-50%);background:var(--bg);color:var(--t1);padding:2px 8px;border-radius:6px;font-size:7.5px;white-space:nowrap;border:1px solid var(--glass-border)}
.theme-option:hover .tooltip{display:block}
.to-dark-glass{background:linear-gradient(135deg,#0a0a1a,#1a1a3a)}
.to-light-glass{background:linear-gradient(135deg,#d0d5e0,#eef2f8)}
.to-dark-matte{background:linear-gradient(135deg,#0a0a0f,#1a1a2a)}
.to-light-matte{background:linear-gradient(135deg,#e0e0e8,#f5f5fa)}
.to-moonlight{background:linear-gradient(135deg,#0a0e1a,#1a2a4a)}

/* ── کارت اطلاعات ── */
.sub-info{background:var(--card);backdrop-filter:blur(28px);border:1px solid var(--glass-border);border-radius:var(--radius);padding:28px 30px 24px;margin-bottom:20px;box-shadow:var(--shadow);position:relative;overflow:hidden;transition:var(--transition)}
.sub-info::before{content:'';position:absolute;top:-80px;right:-80px;width:280px;height:280px;background:radial-gradient(circle,var(--accent),transparent 70%);opacity:0.03;pointer-events:none;animation:pulseGlow 12s ease-in-out infinite}
@keyframes pulseGlow{0%,100%{transform:scale(1);opacity:0.02}50%{transform:scale(1.2);opacity:0.05}}
.sub-eyebrow{font-size:10px;font-weight:700;color:var(--accent2);text-transform:uppercase;letter-spacing:0.12em;margin-bottom:8px;display:flex;align-items:center;gap:6px}
.sub-name{font-size:26px;font-weight:800;color:var(--t1);margin-bottom:6px;letter-spacing:-0.02em}
.sub-desc{font-size:13px;color:var(--t2);line-height:1.8;margin-bottom:14px}
.sub-meta-row{font-size:11px;color:var(--t3);margin-bottom:14px;display:flex;align-items:center;gap:8px;flex-wrap:wrap}
.sub-sub-box{background:var(--accent-d);border:1px solid rgba(91,141,239,0.08);border-radius:14px;padding:12px 16px;display:flex;align-items:center;gap:10px;flex-wrap:wrap}
...
.stat-card{background:var(--card);backdrop-filter:blur(16px);border:1px solid var(--glass-border);border-radius:16px;padding:18px 20px;transition:var(--transition);position:relative;overflow:hidden}
.stat-card:hover{border-color:rgba(91,141,239,0.08);transform:translateY(-2px);box-shadow:0 8px 32px rgba(0,0,0,0.2)}
.stat-card .stat-icon{width:32px;height:32px;border-radius:10px;background:var(--accent-d);color:var(--accent);display:flex;align-items:center;justify-content:center;font-size:14px;margin-bottom:10px}
.stat-card:nth-child(2) .stat-icon{background:var(--green-bg);color:var(--green-t)}
.stat-card:nth-child(3) .stat-icon{background:var(--purple-bg);color:var(--purple-t)}
.stat-label{font-size:9px;color:var(--t3);font-weight:700;text-transform:uppercase;letter-spacing:0.07em;margin-bottom:6px}
.stat-val{font-size:24px;font-weight:800;color:var(--t1);line-height:1;letter-spacing:-0.02em}
...
/* ── دکمه‌ها ── */
.btn{font-family:inherit;font-size:12px;font-weight:700;border-radius:12px;padding:8px 16px;cursor:pointer;display:inline-flex;align-items:center;gap:6px;border:none;transition:all 0.3s cubic-bezier(0.34,1.56,0.64,1);white-space:nowrap}
.btn i{font-size:14px}
.btn-p{background:linear-gradient(135deg,var(--accent),#7a5cf0);color:#fff;box-shadow:0 4px 24px rgba(91,141,239,0.15)}
.btn-p:hover{transform:translateY(-2px);box-shadow:0 8px 32px rgba(91,141,239,0.25)}
.btn-g{background:var(--accent-d);color:var(--accent2);border:1px solid rgba(91,141,239,0.06)}
.btn-g:hover{background:rgba(91,141,239,0.08);border-color:rgba(91,141,239,0.12)}
.btn-pur{background:var(--purple-bg);color:var(--purple-t);border:1px solid rgba(157,123,240,0.08)}
.btn-pur:hover{background:rgba(157,123,240,0.12)}
.btn-sm{padding:5px 10px;font-size:10.5px;border-radius:8px}
.btn-sm i{font-size:11px}

/* ── جدول ── */
.cfg-title{font-size:13px;font-weight:800;color:var(--t2);margin-bottom:16px;display:flex;align-items:center;gap:8px;text-transform:uppercase;letter-spacing:0.08em}
.cfg-title i{color:var(--accent);font-size:17px}
.main-table-wrap{overflow-x:auto;border-radius:var(--radius);border:1px solid var(--glass-border);background:var(--card);backdrop-filter:blur(16px);box-shadow:var(--shadow)}
.main-table{width:100%;border-collapse:collapse;font-size:13px;min-width:500px}
.main-table td{padding:10px 16px;border-bottom:1px solid var(--glass-border);vertical-align:middle}
.main-table tr:last-child td{border-bottom:none}
.main-table tr:hover td{background:var(--accent-d)}
.main-table .label-cell{color:var(--t2);font-weight:600;width:130px;white-space:nowrap;font-size:12px}
.main-table .label-cell i{color:var(--accent);font-size:14px;margin-left:8px;width:20px;text-align:center}
.main-table .value-cell{color:var(--t1);font-weight:500;word-break:break-word}
.main-table .value-cell .proto-badge{font-size:9px;padding:2px 10px;border-radius:8px;font-weight:700;background:var(--accent-d);color:var(--accent2)}
.main-table .value-cell .proto-badge.xhttp{background:var(--purple-bg);color:var(--purple-t)}
.main-table .value-cell .status-dot{width:8px;height:8px;border-radius:50%;display:inline-block;margin-left:6px}
.main-table .value-cell .status-dot.active{background:var(--green);box-shadow:0 0 12px rgba(31,184,126,0.3)}
.main-table .value-cell .status-dot.inactive{background:var(--red);box-shadow:0 0 12px rgba(239,68,68,0.2)}
.main-table .value-cell .status-dot.expired{background:var(--amber);box-shadow:0 0 12px rgba(242,163,61,0.2)}
.main-table .value-cell .exp-chip{font-size:9px;padding:2px 10px;border-radius:8px;font-weight:700;white-space:nowrap;display:inline-block}
...
.main-table .value-cell .usage-bar{width:90px;height:5px;border-radius:4px;background:rgba(91,141,239,0.04);overflow:hidden;display:inline-block;vertical-align:middle;margin:0 6px}
.main-table .value-cell .usage-fill{height:100%;border-radius:4px;transition:width 0.6s ease}
.action-group{display:flex;gap:6px;flex-wrap:wrap}
.action-btn{display:inline-flex;align-items:center;gap:5px;padding:5px 12px;border-radius:8px;font-size:11px;font-weight:700;font-family:inherit;border:none;cursor:pointer;transition:all 0.3s cubic-bezier(0.34,1.56,0.64,1);background:var(--glass);border:1px solid var(--glass-border);color:var(--t2)}
.action-btn:hover{transform:translateY(-2px);box-shadow:0 6px 16px rgba(0,0,0,0.15);border-color:rgba(91,141,239,0.15)}
.action-btn i{font-size:13px}
.action-btn.copy-btn{background:var(--accent-d);color:var(--accent2);border-color:rgba(91,141,239,0.06)}
.action-btn.copy-btn:hover{background:rgba(91,141,239,0.12)}
.action-btn.qr-btn{background:var(--purple-bg);color:var(--purple-t);border-color:rgba(157,123,240,0.06)}
.action-btn.qr-btn:hover{background:rgba(157,123,240,0.12)}
.action-btn.eye-btn{background:var(--amber-bg);color:var(--amber-t);border-color:rgba(242,163,61,0.06)}
.action-btn.eye-btn:hover{background:rgba(242,163,61,0.12)}
...
.vless-code{background:rgba(0,0,0,0.08);border-radius:8px;padding:6px 12px;font-size:9.5px;font-family:ui-monospace,monospace;color:var(--accent2);word-break:break-all;line-height:1.7}
.sub-url-box{display:flex;align-items:center;gap:8px;flex-wrap:wrap}
.sub-url-text{font-family:ui-monospace,monospace;font-size:10px;color:var(--accent2);word-break:break-all;flex:1;min-width:140px}
.link-copy-btn{background:none;border:none;color:var(--t3);cursor:pointer;font-size:13px;padding:3px;transition:var(--transition)}
.link-copy-btn:hover{color:var(--accent2)}

/* ── کپی همه ── */
.copy-all-wrap{margin-top:20px;display:flex;justify-content:center}
.copy-all-btn{background:linear-gradient(135deg,var(--accent),#7a5cf0);color:#fff;border:none;border-radius:14px;padding:12px 28px;font-family:inherit;font-size:13px;font-weight:800;cursor:pointer;display:flex;align-items:center;gap:8px;box-shadow:0 6px 28px rgba(91,141,239,0.15);transition:all 0.3s cubic-bezier(0.34,1.56,0.64,1)}
.copy-all-btn:hover{transform:translateY(-2px);box-shadow:0 10px 36px rgba(91,141,239,0.25)}

/* ── صفحه‌ی قفل ── */
.lock-stage{display:flex;align-items:center;justify-content:center;min-height:70vh;padding:20px 0}
.lock-card{background:var(--card);backdrop-filter:blur(28px);border:1px solid var(--glass-border);border-radius:28px;padding:0;text-align:center;max-width:400px;width:100%;box-shadow:var(--shadow);overflow:hidden}
.lock-banner{background:linear-gradient(150deg,rgba(91,141,239,0.04),rgba(91,141,239,0.01) 70%);padding:40px 30px 28px}
.lock-shield{width:68px;height:68px;border-radius:20px;background:var(--glass);border:1px solid var(--glass-border);display:flex;align-items:center;justify-content:center;margin:0 auto 18px;box-shadow:var(--shadow);position:relative}
.lock-shield::after{content:'';position:absolute;inset:-8px;border-radius:24px;border:1px solid var(--glass-border);animation:breathe 2.8s ease-in-out infinite}
@keyframes breathe{0%,100%{transform:scale(1);opacity:0.3}50%{transform:scale(1.08);opacity:0}}
.lock-shield i{font-size:28px;color:var(--accent2)}
.lock-title{font-size:20px;font-weight:800;color:var(--t1);margin-bottom:6px}
.lock-sub{font-size:12px;color:var(--t3);line-height:1.7}
.lock-form{padding:24px 28px 28px}
.lock-field{position:relative;margin-bottom:14px}
.lock-inp{width:100%;padding:13px 44px 13px 44px;border-radius:14px;border:1.5px solid var(--glass-border);background:rgba(255,255,255,0.01);color:var(--t1);font-family:inherit;font-size:14px;outline:none;text-align:center;letter-spacing:0.14em;transition:var(--transition)}
.lock-inp:focus{border-color:var(--accent);box-shadow:0 0 0 4px rgba(91,141,239,0.04)}
...
.lock-err{color:var(--red-t);font-size:11px;margin-bottom:10px;min-height:16px;display:flex;align-items:center;justify-content:center;gap:5px}
.lock-btn{width:100%;justify-content:center;padding:13px;font-size:13px;border-radius:14px}
.lock-footer{padding:12px 28px;border-top:1px solid var(--glass-border);font-size:9.5px;color:var(--t3);display:flex;align-items:center;justify-content:center;gap:6px}
.toast{position:fixed;bottom:24px;left:50%;transform:translateX(-50%) translateY(50px);background:var(--card);backdrop-filter:blur(20px);border:1px solid var(--glass-border);color:var(--t1);border-radius:14px;padding:11px 24px;font-size:12.5px;font-weight:600;opacity:0;transition:all 0.4s cubic-bezier(0.34,1.56,0.64,1);z-index:999;pointer-events:none;display:flex;align-items:center;gap:8px;box-shadow:var(--shadow);white-space:nowrap}
.toast.show{opacity:1;transform:translateX(-50%) translateY(0)}
.toast.ok{border-color:rgba(31,184,126,0.15);background:rgba(31,184,126,0.03);color:var(--green-t)}
.qr-modal{display:none;position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:600;align-items:center;justify-content:center;backdrop-filter:blur(10px);padding:20px}
.qr-modal.open{display:flex}
.qr-box{background:var(--card);backdrop-filter:blur(28px);border:1px solid var(--glass-border);border-radius:24px;padding:28px;text-align:center;max-width:340px;width:100%;box-shadow:var(--shadow);animation:fadeSlide 0.4s ease}
.qr-title{font-size:14px;font-weight:800;margin-bottom:14px;color:var(--t1)}
.qr-img{border-radius:16px;overflow:hidden;margin-bottom:14px}
...
.footer a{color:var(--accent2);font-weight:600;transition:var(--transition)}
...
@media(max-width:640px){
  ...
  .sub-name{font-size:21px}
  .main-table{font-size:12px;min-width:400px}
  .main-table td{padding:8px 10px}
  .main-table .label-cell{width:100px;font-size:11px}
  .action-group{gap:4px}
  .action-btn{padding:4px 8px;font-size:10px}
  .sub-info{padding:18px 16px 16px}
...
<div class="wrap">
  <div class="top">
    <div class="brand">
      ...
      <div><div class="brand-text">X4G</div><div class="brand-sub">پنل اشتراک حرفه‌ای</div></div>
    </div>
...
// ── ترجمه‌ها ──
const LANG={fa:{title:'گروه دسترسی',copySub:'کپی لینک ساب',qrAll:'QR کل',copyLink:'کپی لینک',qr:'QR کد',showLink:'نمایش لینک',hideLink:'پنهان کردن لینک',copied:'کپی شد ✓',online:'آنلاین',loading:'در حال بارگذاری...',error:'خطا در بارگذاری',locked:'این گروه با رمز محافظت شده است',unlock:'ورود به گروه',wrongPass:'رمز اشتباه است',noConfigs:'کانفیگی برای نمایش وجود ندارد',status:'وضعیت',name:'نام',protocol:'پروتکل',usage:'مصرف',quota:'سهمیه',expiry:'انقضا',active:'فعال',inactive:'غیرفعال',expired:'منقضی',unlimited:'نامحدود',days:'روز',lastUpdate:'آخرین بروزرسانی',liveConns:'اتصالات زنده'},
en:{title:'Access Group',copySub:'Copy Subscription Link',qrAll:'All QR',copyLink:'Copy Link',qr:'QR Code',showLink:'Show Link',hideLink:'Hide Link',copied:'Copied ✓',online:'Online',loading:'Loading...',error:'Error loading',locked:'This group is password protected',unlock:'Unlock Group',wrongPass:'Wrong password',noConfigs:'No configs to display',status:'Status',name:'Name',protocol:'Protocol',usage:'Usage',quota:'Quota',expiry:'Expiry',active:'Active',inactive:'Inactive',expired:'Expired',unlimited:'Unlimited',days:'days',lastUpdate:'Last Update',liveConns:'Live Connections'},
ru:{title:'Группа доступа',copySub:'Копировать ссылку подписки',qrAll:'Общий QR',copyLink:'Копировать ссылку',qr:'QR код',showLink:'Показать ссылку',hideLink:'Скрыть ссылку',copied:'Скопировано ✓',online:'Онлайн',loading:'Загрузка...',error:'Ошибка загрузки',locked:'Группа защищена паролем',unlock:'Войти в группу',wrongPass:'Неверный пароль',noConfigs:'Нет конфигураций для отображения',status:'Статус',name:'Название',protocol:'Протокол',usage:'Использовано',quota:'Квота',expiry:'Срок',active:'Активен',inactive:'Неактивен',expired:'Истек',unlimited:'Безлимит',days:'дней',lastUpdate:'Последнее обновление',liveConns:'Активные подключения'}};

function t(key){return LANG[currentLang][key]||key;}

function setTheme(theme){currentTheme=theme;document.documentElement.setAttribute('data-theme',theme);document.querySelectorAll('.theme-option').forEach(el=>el.classList.toggle('active',el.classList.contains('to-'+theme.replace('-','-'))));localStorage.setItem('x4g-pub-theme',theme);}
const savedTheme=localStorage.getItem('x4g-pub-theme');if(savedTheme)setTheme(savedTheme);

function applyThemeMode(dark){isDark=dark;const current=document.documentElement.getAttribute('data-theme')||'dark-glass';if(dark){if(current.startsWith('light-')){setTheme(current.replace('light-','dark-'));}else if(current==='light-matte')setTheme('dark-matte');else setTheme('dark-glass');}else{if(current.startsWith('dark-')){setTheme(current.replace('dark-','light-'));}else if(current==='dark-matte')setTheme('light-matte');else setTheme('light-glass');}localStorage.setItem('x4g-pub-theme-mode',dark?'dark':'light');}
function toggleThemeMode(){applyThemeMode(!isDark);}
const savedMode=localStorage.getItem('x4g-pub-theme-mode');if(savedMode)applyThemeMode(savedMode==='dark');

function applyLang(lang){currentLang=lang;document.documentElement.lang=lang;document.documentElement.setAttribute('dir',lang==='fa'?'rtl':'ltr');const label=document.getElementById('lang-label');if(label)label.textContent={fa:'English',en:'Русский',ru:'فارسی'}[lang]||'English';localStorage.setItem('x4g-pub-lang',lang);if(window._x4gData)renderContent(window._x4gData);}
function toggleLang(){const order=['fa','en','ru'];const idx=order.indexOf(currentLang);applyLang(order[(idx+1)%order.length]);}
const savedLang=localStorage.getItem('x4g-pub-lang');if(savedLang)applyLang(savedLang);
...
function fmtB(b){if(!b||b===0)return '0 B';if(b<1024)return b+' B';if(b<1024**2)return (b/1024).toFixed(1)+' KB';if(b<1024**3)return (b/1024**2).toFixed(2)+' MB';return (b/1024**3).toFixed(2)+' GB';}
function toFa(n){return String(n).replace(/\d/g,d=>'۰۱۲۳۴۵۶۷۸۹'[d]);}
function protoBadge(p){if(p&&p.startsWith('xhttp'))return '<span class="proto-badge xhttp">XHTTP</span>';return '<span class="proto-badge">VLESS·WS</span>';}
function expChip(exp,expired){if(expired)return `<span class="exp-chip exp-exp">${t('expired')}</span>`;if(!exp)return `<span class="exp-chip exp-inf">${t('unlimited')}</span>`;const d=Math.ceil((new Date(exp)-Date.now())/864e5);if(d<=0)return `<span class="exp-chip exp-exp">${t('expired')}</span>`;if(d<=3)return `<span class="exp-chip exp-warn">${toFa(d)} ${t('days')}</span>`;return `<span class="exp-chip exp-ok">${toFa(d)} ${t('days')}</span>`;}
function statusBadge(active,expired){if(expired)return `<span class="status-dot expired"></span> ${t('expired')}`;if(active)return `<span class="status-dot active"></span> ${t('active')}`;return `<span class="status-dot inactive"></span> ${t('inactive')}`;}
...
function renderLock(name,errMsg=''){document.getElementById('root').innerHTML=`<div class="lock-stage"><div class="lock-card"><div class="lock-banner"><div class="lock-shield"><i class="ti ti-shield-lock"></i></div><div class="lock-title">${esc(name)}</div><div class="lock-sub">${t('locked')}</div></div><div class="lock-form"><div class="lock-err" id="lock-err">${errMsg?'<i class="ti ti-alert-circle"></i> '+esc(errMsg):''}</div><div class="lock-field"><i class="ti ti-lock lock-lockicon"></i><input class="lock-inp" type="password" id="lock-pw" placeholder="••••••••" autofocus><button class="lock-eye" type="button" onclick="togglePwVis()"><i class="ti ti-eye" id="lock-eye-icon"></i></button></div><button class="btn btn-p lock-btn" onclick="submitLock()"><i class="ti ti-lock-open"></i> ${t('unlock')}</button></div><div class="lock-footer"><i class="ti ti-shield-check"></i> ${t('online')}</div></div></div>`;const inp=document.getElementById('lock-pw');inp.addEventListener('keydown',e=>{if(e.key==='Enter')submitLock();});}
...
async function submitLock(){const pw=document.getElementById('lock-pw').value;const data=await loadData(pw);if(data.locked){renderLock(data.name,t('wrongPass'));return;}savedPw=pw;renderContent(data);}

function renderContent(d){window._x4gData=d;const baseSubUrl=d.sub_url||(window.location.protocol+'//'+window.location.host+'/p/'+UUID_KEY);const subUrl=baseSubUrl+(savedPw?'?pw='+encodeURIComponent(savedPw):'');window._x4gSubUrl=subUrl;window._x4gSubName=d.name;window._x4gLinks=d.links.map(l=>({vless:l.vless_link,sub:l.sub_url,label:l.label}));const langAttr=currentLang==='fa'?'fa-IR':(currentLang==='en'?'en-US':'ru-RU');const timeStr=new Date().toLocaleTimeString(langAttr);document.getElementById('root').innerHTML=`
<div class="sub-info">
  <div class="sub-eyebrow"><i class="ti ti-folders"></i> ${t('title')}</div>
  <div class="sub-name">${esc(d.name)}</div>
  ${d.desc?`<div class="sub-desc">${esc(d.desc)}</div>`:''}
  <div class="sub-meta-row"><i class="ti ti-clock"></i> ${t('lastUpdate')}: ${timeStr}</div>
  <div class="sub-sub-box">
    <span class="sub-url-text">${esc(subUrl)}</span>
    <button class="btn btn-pur btn-sm" onclick="navigator.clipboard.writeText(window._x4gSubUrl).then(()=>toast(t('copied'),'ok'))"><i class="ti ti-copy"></i> ${t('copySub')}</button>
    <button class="btn btn-g btn-sm" onclick="showQR(window._x4gSubName+' — '+t('title'),window._x4gSubUrl)"><i class="ti ti-qrcode"></i> ${t('qrAll')}</button>
  </div>
</div>

<div class="stats-bar">
  <div class="stat-card"><div class="stat-icon"><i class="ti ti-link"></i></div><div class="stat-label">${t('active')}</div><div class="stat-val">${toFa(d.links.filter(l=>l.active&&!l.expired).length)}</div><div class="stat-sub">${t('from')} ${toFa(d.links.length)} ${t('configs')}</div></div>
  <div class="stat-card"><div class="stat-icon"><i class="ti ti-plug-connected"></i></div><div class="stat-label">${t('liveConns')}</div><div class="stat-val">${toFa(d.active_connections)}</div><div class="stat-sub"><span class="dot"></span> ${t('online')}</div></div>
  <div class="stat-card"><div class="stat-icon"><i class="ti ti-database"></i></div><div class="stat-label">${t('usage')}</div><div class="stat-val" style="font-size:18px">${esc(d.total_used_fmt)}</div><div class="stat-sub">${t('allConfigs')}</div></div>
</div>

<div class="cfg-title"><i class="ti ti-table"></i> ${t('configs')} (${toFa(d.links.length)})</div>
<div class="main-table-wrap">
  <table class="main-table">
    <tbody>${d.links.map((l,i)=>{const pct=l.limit_bytes===0?0:Math.min(100,(l.used_bytes/l.limit_bytes)*100);const bc=pct>90?'var(--red)':pct>70?'var(--amber)':'var(--green)';const lim=l.limit_bytes===0?'∞':fmtB(l.limit_bytes);return`
    <tr><td class="label-cell"><i class="ti ti-checkbox"></i> ${t('status')}</td><td class="value-cell">${statusBadge(l.active,l.expired)}</td></tr>
    <tr><td class="label-cell"><i class="ti ti-tag"></i> ${t('name')}</td><td class="value-cell">${esc(l.label)}${l.connections>0?` <span style="font-size:10px;color:var(--green-t);margin-right:8px">● ${toFa(l.connections)} ${t('liveConns')}</span>`:''}</td></tr>
    <tr><td class="label-cell"><i class="ti ti-plug-connected"></i> ${t('protocol')}</td><td class="value-cell">${protoBadge(l.protocol)}</td></tr>
    <tr><td class="label-cell"><i class="ti ti-gauge"></i> ${t('usage')}</td><td class="value-cell">${fmtB(l.used_bytes)} <span class="usage-bar"><span class="usage-fill" style="width:${pct}%;background:${bc}"></span></span> <span style="font-size:9px;color:var(--t3)">${pct.toFixed(0)}%</span></td></tr>
    <tr><td class="label-cell"><i class="ti ti-database"></i> ${t('quota')}</td><td class="value-cell">${lim}</td></tr>
    <tr><td class="label-cell"><i class="ti ti-calendar"></i> ${t('expiry')}</td><td class="value-cell">${expChip(l.expires_at,l.expired)}</td></tr>
    <tr><td class="label-cell"><i class="ti ti-tools"></i> </td><td class="value-cell"><div class="action-group"><button class="action-btn copy-btn" onclick="navigator.clipboard.writeText('${esc(l.vless_link)}').then(()=>toast(t('copied'),'ok'))"><i class="ti ti-copy"></i> ${t('copyLink')}</button><button class="action-btn qr-btn" onclick="showQR('${esc(l.label)}','${esc(l.vless_link)}')"><i class="ti ti-qrcode"></i> ${t('qr')}</button><button class="action-btn eye-btn" onclick="toggleVless(${i})" id="vt-${i}"><i class="ti ti-eye"></i> <span id="vt-label-${i}">${t('showLink')}</span></button></div><div class="vless-toggle" id="vw-${i}"><div class="vless-code">${esc(l.vless_link)}</div></div></td></tr>`;}).join('')}</tbody>
  </table>
</div>

<div class="copy-all-wrap"><button class="copy-all-btn" onclick="copyAllConfigs()"><i class="ti ti-clipboard-copy"></i> ${t('copyAll')} (${toFa(d.links.filter(l=>l.active&&!l.expired).length)})</button></div>`;
document.querySelectorAll('[id^="vt-"]').forEach(btn=>{const i=parseInt(btn.id.replace('vt-',''));const wrap=document.getElementById('vw-'+i);const label=document.getElementById('vt-label-'+i);if(btn&&wrap&&label){btn.onclick=function(e){e.stopPropagation();const isOpen=wrap.classList.contains('open');if(isOpen){wrap.classList.remove('open');label.textContent=t('showLink');btn.querySelector('i').className='ti ti-eye';}else{wrap.classList.add('open');label.textContent=t('hideLink');btn.querySelector('i').className='ti ti-eye-off';}}}});setTimeout(()=>autoRefresh(),30000);}

function toggleVless(i){const wrap=document.getElementById('vw-'+i);const btn=document.getElementById('vt-'+i);const label=document.getElementById('vt-label-'+i);if(!wrap||!btn||!label)return;const isOpen=wrap.classList.contains('open');if(isOpen){wrap.classList.remove('open');label.textContent=t('showLink');btn.querySelector('i').className='ti ti-eye';}else{wrap.classList.add('open');label.textContent=t('hideLink');btn.querySelector('i').className='ti ti-eye-off';}}
function copyAllConfigs(){const links=window._x4gLinks||[];if(!links.length){toast(t('noConfigs'),'');return;}const text=links.map(l=>l.vless).join('\n');navigator.clipboard.writeText(text).then(()=>toast(t('allCopied'),'ok'));}
async function autoRefresh(){try{const data=await loadData(savedPw);if(!data.locked)renderContent(data);}catch(e){}}
async function init(){try{const data=await loadData();if(data.locked){renderLock(data.name);return;}renderContent(data);}catch(e){document.getElementById('root').innerHTML='<div class="empty-state" style="color:var(--red-t)"><i class="ti ti-alert-circle"></i>'+t('error')+'</div>';}}
init();
</script>
</body>
</html>"""
