...
def get_public_page_html(uuid_key: str) -> str:
    """صفحه پابلیک ساب — یکپارچه، با دکمه‌های بزرگ و پشتیبانی کشویی"""
...
.action-btn .vless-toggle{display:grid;grid-template-rows:0fr;transition:grid-template-rows 0.25s ease;overflow:hidden;margin-top:4px;width:100%}
.action-btn .vless-toggle.open{grid-template-rows:1fr}
.action-btn .vless-inner{overflow:hidden}
.action-btn .vless-code{background:rgba(0,0,0,0.08);border-radius:6px;padding:5px 10px;font-size:9px;font-family:ui-monospace,monospace;color:var(--accent2);word-break:break-all;line-height:1.6;margin-top:4px;width:100%}
...
/* ── دکمه‌ی کشویی پشتیبانی ── */
.support-dropdown{position:relative;width:100%;z-index:20;direction:rtl}
.support-toggle{width:100%;padding:10px 16px;background:var(--glass);backdrop-filter:blur(12px);border:1px solid var(--glass-border);border-radius:12px;color:var(--t2);font-family:inherit;font-size:12px;font-weight:600;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;transition:all 0.3s cubic-bezier(0.34,1.56,0.64,1)}
.support-toggle:hover{background:var(--accent-d);color:var(--accent2);border-color:rgba(91,141,239,0.15);transform:translateY(-2px)}
.support-toggle i:first-child{color:var(--accent);font-size:17px}
.support-toggle #supportArrow{transition:transform 0.4s cubic-bezier(0.34,1.56,0.64,1);font-size:15px;margin-right:auto}
.support-toggle.open #supportArrow{transform:rotate(180deg)}
.support-menu{display:grid;grid-template-rows:0fr;transition:grid-template-rows 0.4s cubic-bezier(0.34,1.56,0.64,1);overflow:hidden;opacity:0;transform:translateY(-8px);transition:all 0.4s cubic-bezier(0.34,1.56,0.64,1)}
.support-menu.open{grid-template-rows:1fr;opacity:1;transform:translateY(0)}
.support-menu-inner{overflow:hidden;background:var(--glass);backdrop-filter:blur(16px);border:1px solid var(--glass-border);border-radius:12px;padding:6px;margin-top:6px;display:flex;flex-direction:column;gap:2px}
.support-item{display:flex;align-items:center;gap:10px;padding:8px 12px;border-radius:8px;color:var(--t2);font-size:12px;transition:var(--transition);cursor:default}
.support-item:hover{background:var(--accent-d)}
.support-item i{color:var(--accent2);font-size:15px;width:20px;text-align:center;flex-shrink:0}
.support-item a{color:var(--t1);text-decoration:none;font-weight:600;transition:var(--transition);flex:1}
.support-item a:hover{color:var(--accent2)}
.support-divider{height:1px;background:var(--glass-border);margin:2px 8px}
.support-version{display:flex;align-items:center;justify-content:center;gap:6px;padding:6px 0 2px;font-size:9.5px;color:var(--t3)}
              ...
              <tr>
                ...
                <td class="value-cell">
                  <div class="action-group">
                    <button class="action-btn copy-btn" onclick="navigator.clipboard.writeText('${esc(l.vless_link)}').then(()=>toast('لینک کپی شد ✓','ok'))">
                      <i class="ti ti-copy"></i> کپی لینک
                    </button>
                    <button class="action-btn qr-btn" onclick="showQR('${esc(l.label)}', '${esc(l.vless_link)}')">
                      <i class="ti ti-qrcode"></i> QR کد
                    </button>
                    <button class="action-btn eye-btn" onclick="toggleVless(${i})" id="vt-${i}">
                      <i class="ti ti-eye"></i> <span id="vt-label-${i}">نمایش لینک</span>
                    </button>
                  </div>
          ...
          <tbody>
            <tr>
              ...
              <td class="value-cell">
                <div class="support-dropdown" id="supportDropdown">
                  <button class="support-toggle" onclick="toggleSupport()">
                    <i class="ti ti-headset"></i> ارتباط با پشتیبانی
                    <i class="ti ti-chevron-down" id="supportArrow"></i>
                  </button>
                  <div class="support-menu" id="supportMenu">
                    <div class="support-menu-inner">
  ...
  document.querySelectorAll('[id^="vt-"]').forEach(btn => {
    ...
    if(btn && wrap && label) {
      btn.onclick = function(e) {
        ...
        label.textContent = open ? 'پنهان کردن لینک' : 'نمایش لینک';
        btn.querySelector('i').className = open ? 'ti ti-eye-off' : 'ti ti-eye';
      };
    }
  });
