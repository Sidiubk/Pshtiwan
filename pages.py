...
def get_public_page_html(uuid_key: str) -> str:
    """صفحه پابلیک ساب — طراحی یکپارچه جدولی عمودی"""
...
.main-table .value-cell .actions-wrap{display:flex;gap:4px;flex-wrap:wrap}
.main-table .value-cell .vless-toggle{display:grid;grid-template-rows:0fr;transition:grid-template-rows 0.25s ease;overflow:hidden;margin-top:4px}
.main-table .value-cell .vless-toggle.open{grid-template-rows:1fr}
.main-table .value-cell .vless-inner{overflow:hidden}
.main-table .value-cell .vless-code{background:rgba(0,0,0,0.08);border-radius:6px;padding:5px 10px;font-size:9px;font-family:ui-monospace,monospace;color:var(--accent2);word-break:break-all;line-height:1.6;margin-top:4px}
.main-table .value-cell .btn-sm{padding:4px 8px;font-size:9.5px;border-radius:6px;background:var(--glass);border:1px solid var(--glass-border);color:var(--t2);cursor:pointer;transition:var(--transition);font-family:inherit;display:inline-flex;align-items:center;gap:3px}
.main-table .value-cell .btn-sm:hover{background:var(--accent-d);color:var(--accent2);border-color:rgba(91,141,239,0.12)}
.main-table .value-cell .btn-sm i{font-size:10px}
.main-table .value-cell .btn-sm.primary{background:linear-gradient(135deg,var(--accent),#7a5cf0);color:#fff;border:none}
.main-table .value-cell .btn-sm.primary:hover{transform:translateY(-1px);box-shadow:0 4px 12px rgba(91,141,239,0.15)}
.main-table .value-cell .sub-url-box{display:flex;align-items:center;gap:6px;flex-wrap:wrap}
.main-table .value-cell .sub-url-text{font-family:ui-monospace,monospace;font-size:9.5px;color:var(--accent2);word-break:break-all;flex:1;min-width:120px}
.main-table .value-cell .link-copy-btn{background:none;border:none;color:var(--t3);cursor:pointer;font-size:12px;padding:2px;transition:var(--transition)}
.main-table .value-cell .link-copy-btn:hover{color:var(--accent2)}
.main-table .value-cell .sub-actions{display:flex;gap:4px;flex-wrap:wrap}
...
.footer{text-align:center;padding-top:20px;font-size:10px;color:var(--t3)}
  ...
  document.getElementById('root').innerHTML = `
    <div class="main-table-wrap">
      <table class="main-table">
        <tbody>
          <tr>
            <td class="label-cell"><i class="ti ti-folders"></i> گروه دسترسی</td>
          ...
          <tr>
            <td class="label-cell"><i class="ti ti-clock"></i> آخرین بروزرسانی</td>
            <td class="value-cell">${new Date().toLocaleTimeString('fa-IR')}</td>
          </tr>
          <tr>
            ...
            <td class="value-cell">
              <div class="sub-url-box">
                ...
                <button class="link-copy-btn" onclick="navigator.clipboard.writeText(window._x4gSubUrl).then(()=>toast('لینک ساب کپی شد ✓','ok'))" title="کپی لینک"><i class="ti ti-copy"></i></button>
                <button class="link-copy-btn" onclick="showQR(window._x4gSubName + ' — کل گروه', window._x4gSubUrl)" title="QR کل"><i class="ti ti-qrcode"></i></button>
              </div>
            </td>
          </tr>
          <tr>
            <td class="label-cell"><i class="ti ti-checkbox"></i> کانفیگ‌های فعال</td>
            <td class="value-cell">${toFa(activeCount)} از ${toFa(d.links.length)} کانفیگ</td>
          </tr>
          <tr>
            <td class="label-cell"><i class="ti ti-plug-connected"></i> اتصالات زنده</td>
            <td class="value-cell">${toFa(d.active_connections)} <span class="dot"></span> آنلاین</td>
          </tr>
          <tr>
            <td class="label-cell"><i class="ti ti-database"></i> کل مصرف</td>
            <td class="value-cell">${esc(d.total_used_fmt)}</td>
          </tr>
        </tbody>
      </table>
    </div>
              ...
              <tr>
                ...
                <td class="value-cell">
                  <div class="actions-wrap">
                    <button class="btn-sm" onclick="navigator.clipboard.writeText('${esc(l.vless_link)}').then(()=>toast('لینک کپی شد ✓','ok'))" title="کپی لینک"><i class="ti ti-copy"></i> کپی</button>
                    <button class="btn-sm" onclick="showQR('${esc(l.label)}', '${esc(l.vless_link)}')" title="QR"><i class="ti ti-qrcode"></i> QR</button>
                    <button class="btn-sm" onclick="toggleVless(${i})" id="vt-${i}" title="نمایش لینک"><i class="ti ti-eye"></i></button>
                  </div>
    ...
    <div style="margin-top:14px">
      <div class="main-table-wrap">
        <table class="main-table">
          <tbody>
            <tr>
              <td class="label-cell"><i class="ti ti-clipboard-copy"></i> کپی همه</td>
              <td class="value-cell">
                <button class="btn-sm primary" onclick="copyAllConfigs()">کپی همه‌ی کانفیگ‌ها (${toFa(activeCount)})</button>
              </td>
            </tr>
            <tr>
              <td class="label-cell"><i class="ti ti-headset"></i> پشتیبانی</td>
              <td class="value-cell">
                <a href="https://t.me/X4GHUB" target="_blank" style="color:var(--accent2);font-weight:600;text-decoration:none">@X4GHUB</a>
              </td>
            </tr>
          </tbody>
        </table>
