...
def get_public_page_html(uuid_key: str) -> str:
    """صفحه پابلیک ساب — طراحی جدولی، حرفه‌ای، دو زبانه، با تم روشن/تاریک"""
...
<style>
/* ========================================
   طراحی جدولی، حرفه‌ای و دو زبانه — X4G v9.8
   ======================================== */
...
[data-theme="light"]{
  ...
  --green:#0E9A6A;
  --green-bg:rgba(14,154,106,0.05);
  --green-t:#0A7553;
  --red:#DC2626;
  --red-bg:rgba(220,38,38,0.05);
  --red-t:#A51E1E;
  --amber:#C97A12;
  --amber-bg:rgba(201,122,18,0.05);
  --amber-t:#8F5A0C;
  --purple:#7350D6;
  --purple-bg:rgba(115,80,214,0.05);
  --purple-t:#5A3CAD;
  --t1:#111827;
  --t2:#48577a;
  --t3:#8694b0;
...
.wrap{position:relative;z-index:10;max-width:860px;margin:0 auto;padding:24px 16px 64px;animation:fadeSlide 0.8s ease}
@keyframes fadeSlide{from{opacity:0;transform:translateY(30px)}to{opacity:1;transform:translateY(0)}}

/* ── هدر ── */
.top{display:flex;align-items:center;justify-content:space-between;margin-bottom:26px;gap:10px;flex-wrap:wrap}
.brand{display:flex;align-items:center;gap:11px;min-width:0}
.brand-img{width:40px;height:40px;border-radius:50%;overflow:hidden;border:1px solid var(--glass-border);box-shadow:0 0 20px rgba(91,141,239,0.06);flex-shrink:0}
.brand-img img{width:100%;height:100%;object-fit:cover}
...
.icon-btn{width:36px;height:36px;border-radius:11px;background:var(--glass);backdrop-filter:blur(12px);border:1px solid var(--glass-border);color:var(--t2);display:flex;align-items:center;justify-content:center;font-size:16px;cursor:pointer;transition:var(--transition)}
.icon-btn:hover{background:var(--accent-d);color:var(--accent2);border-color:rgba(91,141,239,0.12);transform:translateY(-1px)}
.lang-btn{font-size:12px;font-weight:700;padding:0 12px;width:auto;gap:4px;background:var(--glass);backdrop-filter:blur(12px);border:1px solid var(--glass-border);color:var(--t2);border-radius:11px;height:36px;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:var(--transition)}
.lang-btn:hover{background:var(--accent-d);color:var(--accent2);border-color:rgba(91,141,239,0.12)}

/* ── کارت اطلاعات ── */
.sub-info{background:var(--glass);backdrop-filter:blur(24px);border:1px solid var(--glass-border);border-radius:var(--radius);padding:24px 26px 22px;margin-bottom:16px;box-shadow:var(--shadow);position:relative;overflow:hidden;transition:var(--transition)}
.sub-info::before{content:'';position:absolute;top:0;right:0;width:160px;height:160px;background:radial-gradient(circle at top right,rgba(91,141,239,0.04),transparent 70%);pointer-events:none}
...
.sub-name{font-size:23px;font-weight:800;color:var(--t1);margin-bottom:6px;letter-spacing:-0.02em}
...
.sub-sub-box{background:var(--accent-d);border:1px solid var(--glass-border);border-radius:13px;padding:12px 16px;display:flex;align-items:center;gap:9px;flex-wrap:wrap}
...
.stat-card{background:var(--glass);backdrop-filter:blur(16px);border:1px solid var(--glass-border);border-radius:16px;padding:16px 18px;transition:var(--transition)}
.stat-card:hover{border-color:rgba(91,141,239,0.08);transform:translateY(-2px)}
.stat-label{font-size:9px;color:var(--t3);font-weight:700;text-transform:uppercase;letter-spacing:0.07em;margin-bottom:7px}
.stat-val{font-size:22px;font-weight:800;color:var(--t1);line-height:1;letter-spacing:-0.01em}
.stat-sub{font-size:9.5px;color:var(--t3);margin-top:6px}

/* ── نوار کپی همه ── */
.copy-all-bar{display:flex;align-items:center;gap:12px;background:linear-gradient(120deg,var(--accent),#7a5cf0);border-radius:18px;padding:14px 20px;margin-bottom:18px;box-shadow:0 8px 28px rgba(91,141,239,0.12);flex-wrap:wrap;transition:var(--transition)}
...
.copy-all-sub{font-size:10px;color:rgba(255,255,255,0.7);margin-top:3px}
.copy-all-btn{background:#fff;color:#1D4ED8;border:none;border-radius:12px;padding:10px 20px;font-family:inherit;font-size:12.5px;font-weight:800;cursor:pointer;display:flex;align-items:center;gap:6px;transition:all 0.3s cubic-bezier(0.34,1.56,0.64,1);white-space:nowrap}
.copy-all-btn:hover{transform:translateY(-2px);box-shadow:0 8px 24px rgba(0,0,0,0.1)}
.copy-all-btn:active{transform:translateY(0) scale(0.96)}

/* ── جدول کانفیگ‌ها ── */
.cfg-title{font-size:12px;font-weight:800;color:var(--t2);margin-bottom:14px;display:flex;align-items:center;gap:6px;text-transform:uppercase;letter-spacing:0.07em}
.cfg-title i{color:var(--accent);font-size:15px}

/* ── جدول ── */
.cfg-table-wrap{overflow-x:auto;border-radius:var(--radius);border:1px solid var(--glass-border);background:var(--glass);backdrop-filter:blur(16px);box-shadow:var(--shadow)}
.cfg-table{width:100%;border-collapse:collapse;font-size:12.5px;min-width:700px}
.cfg-table thead{background:var(--accent-d);border-bottom:1px solid var(--glass-border)}
.cfg-table th{padding:12px 14px;text-align:right;font-size:10px;font-weight:700;color:var(--t2);text-transform:uppercase;letter-spacing:0.06em;white-space:nowrap}
.cfg-table td{padding:12px 14px;border-bottom:1px solid var(--glass-border);vertical-align:middle;color:var(--t1)}
.cfg-table tr:last-child td{border-bottom:none}
.cfg-table tr:hover td{background:var(--accent-d)}
.cfg-table .cfg-status-dot{width:8px;height:8px;border-radius:50%;display:inline-block;margin-left:6px;flex-shrink:0}
.cfg-table .dot-active{background:var(--green);box-shadow:0 0 8px rgba(31,184,126,0.3)}
.cfg-table .dot-inactive{background:var(--red);box-shadow:0 0 8px rgba(239,68,68,0.2)}
.cfg-table .dot-expired{background:var(--amber);box-shadow:0 0 8px rgba(242,163,61,0.2)}
.cfg-table .cfg-label-cell{font-weight:700;color:var(--t1);display:flex;align-items:center;gap:8px}
.cfg-table .cfg-proto{font-size:8.5px;padding:2px 8px;border-radius:6px;font-weight:700;white-space:nowrap;background:var(--accent-d);color:var(--accent2)}
.cfg-table .cfg-proto.xhttp{background:var(--purple-bg);color:var(--purple-t)}
.cfg-table .cfg-usage-bar{width:100px;height:5px;border-radius:4px;background:rgba(91,141,239,0.06);overflow:hidden;display:inline-block;vertical-align:middle;margin:0 6px}
.cfg-table .cfg-usage-fill{height:100%;border-radius:4px;transition:width 0.6s ease}
.cfg-table .cfg-actions-cell{display:flex;gap:4px;flex-wrap:wrap}
.cfg-table .btn-sm{padding:4px 8px;font-size:9.5px;border-radius:6px;background:var(--glass);border:1px solid var(--glass-border);color:var(--t2);cursor:pointer;transition:var(--transition);font-family:inherit}
.cfg-table .btn-sm:hover{background:var(--accent-d);color:var(--accent2);border-color:rgba(91,141,239,0.12)}
.cfg-table .btn-sm i{font-size:11px}
.cfg-table .btn-sm.primary{background:linear-gradient(135deg,var(--accent),#7a5cf0);color:#fff;border:none}
.cfg-table .btn-sm.primary:hover{transform:translateY(-1px);box-shadow:0 4px 12px rgba(91,141,239,0.2)}
.cfg-table .exp-chip{font-size:8.5px;padding:2px 8px;border-radius:6px;font-weight:700;white-space:nowrap;display:inline-block}
.cfg-table .exp-ok{background:var(--green-bg);color:var(--green-t)}
.cfg-table .exp-warn{background:var(--amber-bg);color:var(--amber-t)}
.cfg-table .exp-exp{background:var(--red-bg);color:var(--red-t)}
.cfg-table .exp-inf{background:var(--accent-d);color:var(--accent2)}

/* ── دکمه‌ها ── */
.btn{font-family:inherit;font-size:11.5px;font-weight:700;border-radius:10px;padding:8px 15px;cursor:pointer;display:inline-flex;align-items:center;gap:5px;border:none;transition:all 0.25s cubic-bezier(0.34,1.56,0.64,1);white-space:nowrap}
.btn i{font-size:13px}
.btn-p{background:linear-gradient(135deg,var(--accent),#7a5cf0);color:#fff;box-shadow:0 4px 20px rgba(91,141,239,0.12)}
.btn-p:hover{transform:translateY(-2px);box-shadow:0 8px 28px rgba(91,141,239,0.2)}
.btn-g{background:var(--accent-d);color:var(--accent2);border:1px solid rgba(91,141,239,0.06)}
.btn-g:hover{background:rgba(91,141,239,0.08)}
.btn-pur{background:var(--purple-bg);color:var(--purple-t);border:1px solid rgba(157,123,240,0.08)}
.btn-pur:hover{background:rgba(157,123,240,0.1)}

/* ── صفحه‌ی قفل ── */
.lock-stage{display:flex;align-items:center;justify-content:center;min-height:78vh;padding:20px 0}
.lock-card{background:var(--glass);backdrop-filter:blur(28px);border:1px solid var(--glass-border);border-radius:28px;padding:0;text-align:center;max-width:400px;width:100%;box-shadow:var(--shadow);overflow:hidden;position:relative}
.lock-banner{background:linear-gradient(150deg,rgba(91,141,239,0.04),rgba(91,141,239,0.01) 70%);padding:40px 32px 28px}
.lock-shield{width:68px;height:68px;border-radius:20px;background:var(--glass);border:1px solid var(--glass-border);display:flex;align-items:center;justify-content:center;margin:0 auto 18px;box-shadow:var(--shadow)}
...
.lock-title{font-size:19px;font-weight:800;color:var(--t1);margin-bottom:6px}
...
.lock-inp{width:100%;padding:14px 46px 14px 46px;border-radius:14px;border:1.5px solid var(--glass-border);background:rgba(255,255,255,0.01);color:var(--t1);font-family:inherit;font-size:14px;outline:none;text-align:center;letter-spacing:0.14em;transition:var(--transition)}
...
.lock-footer{padding:14px 32px;border-top:1px solid var(--glass-border);font-size:10px;color:var(--t3);display:flex;align-items:center;justify-content:center;gap:6px}
...
.toast{position:fixed;bottom:24px;left:50%;transform:translateX(-50%) translateY(50px);background:var(--glass);backdrop-filter:blur(20px);border:1px solid var(--glass-border);color:var(--t1);border-radius:14px;padding:11px 22px;font-size:12.5px;font-weight:600;opacity:0;transition:all 0.4s cubic-bezier(0.34,1.56,0.64,1);z-index:999;pointer-events:none;display:flex;align-items:center;gap:8px;box-shadow:var(--shadow);white-space:nowrap}
.toast.show{opacity:1;transform:translateX(-50%) translateY(0)}
.toast.ok{border-color:rgba(31,184,126,0.15);background:rgba(31,184,126,0.03);color:var(--green-t)}
.qr-modal{display:none;position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:600;align-items:center;justify-content:center;backdrop-filter:blur(8px);padding:20px}
.qr-modal.open{display:flex}
.qr-box{background:var(--glass);backdrop-filter:blur(28px);border:1px solid var(--glass-border);border-radius:24px;padding:28px;text-align:center;max-width:360px;width:100%;box-shadow:var(--shadow);animation:fadeSlide 0.4s ease}
...
.footer a{color:var(--accent2);font-weight:700;transition:var(--transition)}
.footer a:hover{color:var(--accent)}
.lang-footer{display:flex;align-items:center;justify-content:center;gap:8px;margin-top:10px;font-size:10.5px;color:var(--t3)}
.lang-footer .lang-switch{background:none;border:none;color:var(--accent2);font-weight:700;cursor:pointer;font-family:inherit;font-size:10.5px;padding:2px 8px;border-radius:6px;transition:var(--transition)}
.lang-footer .lang-switch:hover{background:var(--accent-d);color:var(--accent)}
...
@media(max-width:640px){
  ...
  .cfg-table{font-size:11px;min-width:600px}
  .cfg-table th,.cfg-table td{padding:8px 10px}
}
@keyframes spin{to{transform:rotate(360deg)}}
</style>
</head>
...
<div class="wrap">
  ...
  <div class="footer">
    ...
    <div class="lang-footer">
      <span id="lang-footer-label">زبان</span>
      <button class="lang-switch" onclick="toggleLang()" id="lang-switch-btn">English</button>
    </div>
    <div style="margin-top:4px;font-size:9.5px;color:var(--t3)">X4G v9.8</div>
  </div>
</div>
<script>
...
const LANG = {
  fa: {
    ...
    qrAll: 'QR کل',
    copyAll: 'کپی همه‌ی کانفیگ‌ها',
    copyAllDesc: 'تمام لینک‌های فعال این گروه را یک‌جا کپی کن',
    copyAllBtn: 'کپی همه',
    ...
    showLink: 'نمایش لینک',
    hideLink: 'پنهان کردن',
    noConfigs: 'هیچ کانفیگی در این گروه وجود ندارد',
    ...
    copied: 'کپی شد ✓',
    allCopied: 'همه‌ی کانفیگ‌ها کپی شد ✓',
    support: 'پشتیبانی',
    lang: 'زبان',
    daysLeft: 'روز مانده',
    noExpiry: 'بدون انقضا',
  ...
  en: {
    ...
    copyAllDesc: 'Copy all active links of this group at once',
    copyAllBtn: 'Copy All',
    ...
    showLink: 'Show Link',
    hideLink: 'Hide',
    noConfigs: 'No configs in this group',
    loading: 'Loading...',
    error: 'Error loading',
...
function applyLang(lang){
  ...
  const footerLabel = document.getElementById('lang-footer-label');
  if(label) label.textContent = lang === 'fa' ? 'English' : 'فارسی';
  if(btn) btn.textContent = lang === 'fa' ? 'English' : 'فارسی';
  if(footerLabel) footerLabel.textContent = lang === 'fa' ? 'زبان' : 'Language';
...
function protoChip(p){
  if(p && p.startsWith('xhttp')) return '<span class="cfg-proto xhttp">XHTTP</span>';
  return '<span class="cfg-proto">VLESS·WS</span>';
}
function expChip(exp, expired){
  if(expired) return `<span class="exp-chip exp-exp">${t('expired')}</span>`;
  if(!exp) return `<span class="exp-chip exp-inf">${t('noExpiry')}</span>`;
  const d = Math.ceil((new Date(exp) - Date.now()) / 864e5);
  if(d <= 0) return `<span class="exp-chip exp-exp">${t('expired')}</span>`;
  if(d <= 3) return `<span class="exp-chip exp-warn">${d} ${t('days')}</span>`;
  return `<span class="exp-chip exp-ok">${d} ${t('days')}</span>`;
}
function statusBadge(active, expired){
  if(expired) return `<span class="cfg-status-dot dot-expired"></span><span style="color:var(--amber-t)">${t('expired')}</span>`;
  if(active) return `<span class="cfg-status-dot dot-active"></span><span style="color:var(--green-t)">${t('active')}</span>`;
  return `<span class="cfg-status-dot dot-inactive"></span><span style="color:var(--red-t)">${t('inactive')}</span>`;
}
...
function renderContent(d){
  ...
  document.getElementById('root').innerHTML = `
    <div class="sub-info">
      ...
      <div class="sub-meta-row"><i class="ti ti-clock"></i> ${new Date().toLocaleTimeString(currentLang === 'fa' ? 'fa-IR' : 'en-US')}</div>
      <div class="sub-sub-box">
        ...
        <button class="btn btn-pur" style="padding:7px 12px;font-size:10.5px"
          onclick="navigator.clipboard.writeText(window._x4gSubUrl).then(()=>toast(t('copied'),'ok'))">
          <i class="ti ti-copy"></i> ${t('copySub')}
        </button>
        <button class="btn btn-g" style="padding:7px 12px;font-size:10.5px"
          onclick="showQR(window._x4gSubName + ' — ' + t('allConfigs'), window._x4gSubUrl)">
          <i class="ti ti-qrcode"></i> ${t('qrAll')}
        </button>
      </div>
    </div>
    ...
    <div class="copy-all-bar">
      <div class="copy-all-text">
        <div class="copy-all-title"><i class="ti ti-copy"></i> ${t('copyAll')}</div>
        <div class="copy-all-sub">${t('copyAllDesc')}</div>
      </div>
      <button class="copy-all-btn" onclick="copyAllConfigs()"><i class="ti ti-clipboard-copy"></i> ${t('copyAllBtn')} (${toFa(activeCount)})</button>
    </div>

    <div class="stats-bar">
      <div class="stat-card">
        <div class="stat-label">${t('activeConfigs')}</div>
        <div class="stat-val">${toFa(activeCount)}</div>
        <div class="stat-sub">${t('of')} ${toFa(d.links.length)} ${t('configs')}</div>
      </div>
      <div class="stat-card">
        <div class="stat-label">${t('liveConns')}</div>
        <div class="stat-val">${toFa(d.active_connections)}</div>
        <div class="stat-sub" style="color:var(--green-t);display:flex;align-items:center;gap:4px"><span class="dot"></span> ${t('online')}</div>
      </div>
      <div class="stat-card">
        <div class="stat-label">${t('totalUsage')}</div>
        <div class="stat-val" style="font-size:17px;margin-top:3px">${esc(d.total_used_fmt)}</div>
        <div class="stat-sub">${t('allConfigs')}</div>
      </div>
    </div>
      ...
      <table class="cfg-table">
        ...
        <tbody>
          ${d.links.map((l, i) => {
            ...
            return `
            <tr>
              <td>${statusBadge(l.active, l.expired)}</td>
              <td><span class="cfg-label-cell">${esc(l.label)}${l.connections > 0 ? ` <span class="conn-chip" style="font-size:8.5px;padding:2px 6px">${toFa(l.connections)} ${t('liveConns')}</span>` : ''}</span></td>
              <td>${protoChip(l.protocol)}</td>
              <td>
                <span>${fmtB(l.used_bytes)}</span>
                <span class="cfg-usage-bar"><span class="cfg-usage-fill" style="width:${pct}%;background:${bc}"></span></span>
              ...
              <td>
                <div class="cfg-actions-cell">
                  <button class="btn-sm" onclick="navigator.clipboard.writeText('${esc(l.vless_link)}').then(()=>toast(t('copied'),'ok'))" title="${t('copyLink')}"><i class="ti ti-copy"></i></button>
                  <button class="btn-sm" onclick="showQR('${esc(l.label)}', '${esc(l.vless_link)}')" title="${t('qr')}"><i class="ti ti-qrcode"></i></button>
                  <button class="btn-sm" onclick="toggleLink(${i})" id="vt-${i}" title="${t('showLink')}"><i class="ti ti-eye"></i></button>
                </div>
                <div style="margin-top:6px;display:grid;grid-template-rows:0fr;transition:grid-template-rows 0.25s ease;overflow:hidden" id="vw-${i}">
                  <div style="overflow:hidden">
                    <div style="background:rgba(0,0,0,0.08);border-radius:6px;padding:6px 10px;font-size:9.5px;font-family:ui-monospace,monospace;color:var(--accent2);word-break:break-all;line-height:1.6;margin-top:4px">${esc(l.vless_link)}</div>
                  </div>
                </div>
              </td>
            </tr>
  ...
  document.querySelectorAll('[id^="vt-"]').forEach(btn => {
    ...
    if(btn && wrap) {
      btn.onclick = function(e) {
        e.stopPropagation();
        const open = wrap.style.gridTemplateRows !== '1fr';
        wrap.style.gridTemplateRows = open ? '1fr' : '0fr';
        btn.title = open ? 'پنهان کردن لینک' : 'نمایش لینک';
      };
    }
  });
...
function toggleLink(i){
  ...
  const open = wrap.style.gridTemplateRows !== '1fr';
  wrap.style.gridTemplateRows = open ? '1fr' : '0fr';
  btn.title = open ? (currentLang === 'fa' ? 'پنهان کردن لینک' : 'Hide Link') : (currentLang === 'fa' ? 'نمایش لینک' : 'Show Link');
}

function copyAllConfigs(){
  const links = window._x4gLinks || [];
  if(!links.length){ toast(t('noConfigs'), ''); return; }
